...
const Workspace: FC<WorkspaceProps> = ({ songData }) => {
  ...
  const songContents = songData?.contents.map((element, index) => {
    switch (element.type) {
      ...
      case "TEXT_BLOCK":
        return <TextBlock key={index} paragraphs={element.data.paragraphs} />;
      case "COMMENT_BLOCK":
        return <CommentBlock key={index} text={element.data.text} />;
      default:
        return null;
    }
  });
